...
void initAnaliseLexica();
void getToken(TOKEN *t);
void rollBackPos(int len);
...
void initAnaliseLexica() {
    ...
    txt = (char *) malloc(tamanho * sizeof(char));
...
void getToken(TOKEN *t) {
    ...
    if (pos < strlen(txt)) {
        ...
        printf("\nArquivo de saida de tokens foi gerado com sucesso com o nome output.txt\n\n");
        (*t).nome = __EOF__;
    }
}

void rollBackPos(int len) {
  	/* Voltar para tras, para que o proximo lookahead, seja igual a lookaheadFollow. */
  	int i;
  	for (i = 0; i < (len - 1); i++) {
  		pos--;
  	}
...
void readFile() {
    printf("Lendo o arquivo de entrada: input.txt\n");
    ...
    if (arquivo_entrada) {
        ...
        fclose(arquivo_entrada);
  		...
  		printf("\nO arquivo input.txt nao se encontra na pasta do projeto.\n");
  		exit(0);
    }
}
...
int needValue(char aux[]) {
    int c;
    c = atoi(aux);
    if (c == __NUMERO_POSITIVO__ || c == __NUMERO_NEGATIVO__ || c == __IDENTIFICADOR__)
      return  1;
    return 0;
}
